...
//!
//! This module deliberately stays dependency-light: proof JSON is parsed with a
//! tiny schema-local parser so the crate does not promote `serde_json` from a
//! dev-dependency into the default runtime graph.
