...
fingerprints identify embedded transport keys. Its canonical source repository is
now [`Blackcat-Informatics/visual-hashing`](https://github.com/Blackcat-Informatics/visual-hashing).
The crate's conformance corpus is generated by a Python reference implementation
and shared across languages, so the renderings are portable beyond Rust.
