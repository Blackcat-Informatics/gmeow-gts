...
For the 1.0 release line, add `gmeow-gts = "1.0.0-rc.1"` to `Cargo.toml`. Optional feature builds
use the standard Cargo shape
`gmeow-gts = { version = "1.0.0-rc.1", default-features = false, features = [...] }`.
...
Install the 1.0 release-line command-line tool with
`go install go.blackcatinformatics.ca/gts/cmd/gts@v1.0.0-rc.1`, or import the same module path
from Go services.
