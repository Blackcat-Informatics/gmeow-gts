...
TypeScript package's browser-safe progressive stream surface, not a current non-materializing
`GTS Streaming Reader` tier claim. The root Node `Read(bytes, allowSegments)` API remains
materializing, and browser code must not rely on the Node-only CLI/filesystem helpers.
