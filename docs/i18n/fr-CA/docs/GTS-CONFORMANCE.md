...
| `ConflictingReifier` | erreur | repli du graphe | Émis UNIQUEMENT pour un terme `k:3` sans `"tt"` (indirection héritée) dont le réificateur lie plus d'un triplet distinct (GTS-SPEC §7.3). Signalé une fois par terme fautif, N'ÉCARTE AUCUNE ligne `reifies`, et ce terme se résout sur la première liaison dans l'ordre du fichier. Plusieurs liaisons `rdf:reifies` sur un même réificateur ne sont PAS un conflit — `rdf:reifies` n'est pas fonctionnelle. | oui | aucun | Baseline Reader |
