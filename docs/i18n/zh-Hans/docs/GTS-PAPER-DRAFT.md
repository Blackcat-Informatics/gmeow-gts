...
RDF 数据集通常通过文本序列化、数据库导出、临时归档和特定于应用的包格式进行交换。这些机制很有用，但它们没有为仅追加的图历史、内容寻址的二进制有效负载、部分可读性和跨语言一致性提供一个小型公共基底。GTS 通过将 RDF 1.2 图状态和被引用的二进制资产编码为由确定性段和帧组成的 CBOR Sequence 来填补这一空缺。每个段会折叠为图状态，帧通过 BLAKE3 内容标识符链接，不受支持或无法访问的有效负载会降级为不透明图节点，而不是消失。当前仓库托管了 Rust、Python、Go、TypeScript、Smalltalk/Pharo 和 Kotlin/JVM 的六个参考引擎，以及用于比较折叠结果和诊断的共享向量清单与语料库。GTS 不是数据库、推理器、本体或共识协议；它是用于持久、可验证图传输的窄腰。
