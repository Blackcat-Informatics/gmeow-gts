...
The Rust `read_to_sink` API is event-emitting reader evidence, not a current Streaming Reader tier
claim: it accepts a byte slice, decodes the item collection, and uses the segment `Graph` path
while emitting events. The TypeScript browser `foldStream(stream, options)` and
