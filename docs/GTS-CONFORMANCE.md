...
| `writer-determinism` | Python generator for all top-level vectors, plus `25b-streamable-compacted` as the streamable compaction byte oracle | Reproducible writer output, deterministic hashes, and deterministic compaction under fixed parameters. |
...
a future manifest explicitly narrows a vector:
...
| `nquads` | Sorted RDF projection lines from the folded graph. Blank-node labels are expected to match the reference renderer unless a future manifest declares isomorphism-only comparison. |

## 5. Vector Manifest Schema

The repository does not yet commit a `vectors/manifest.json`; the current file-pair convention
is the implicit manifest. A portable manifest MUST use this shape when introduced:
